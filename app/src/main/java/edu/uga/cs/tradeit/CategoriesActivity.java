...
public class CategoriesActivity extends AppCompatActivity implements RecyclerAdapter.OnRowAction {
                        ...
                        FirebaseRefs.categories().child(categoryId)
                                ...
                                .runTransaction(new com.google.firebase.database.Transaction.Handler() {
                                    ...
                                    public void onComplete(DatabaseError error, boolean committed, DataSnapshot snapshot) {
                                        if (committed) {
                                            createTransactionForPostedItem(item);
    ...
    private void createTransactionForPostedItem(Item item) {
        String txKey = FirebaseRefs.pendingByUser(item.sellerUid).push().getKey();
        if (txKey != null && item.sellerUid != null) {
            Transaction transaction = new Transaction();
            transaction.id = txKey;
            transaction.itemId = item.id;
            transaction.itemName = item.name;
            transaction.categoryID = item.categoryId;
            transaction.sellerUserID = item.sellerUid;
            transaction.buyerUserID = null; // No buyer yet
            ...
            FirebaseRefs.pendingByUser(item.sellerUid).child(txKey).setValue(transaction)
                    .addOnSuccessListener(aVoid -> {
                    });
        }
    }
    ...
    // User Story 12 - Bidding on items
    private void buyItem(Item item) {
        ...
        if (item.sellerUid == null || item.sellerUid.isEmpty()) {
            toast("Error: Item seller information is missing");
            return;
        }

        String message = item.isFree
                ? "Do you want to bid on this free item?"
                : "Do you want to bid $" + String.format("%.2f", item.price / 100.0) + " on this item?";

        new AlertDialog.Builder(this)
                .setTitle(item.isFree ? "Accept Item" : "Bid on Item")
                .setMessage(message)
                .setPositiveButton(item.isFree ? "Accept" : "Bid", (dialog, which) -> {
                    // CHANGE 4: Find existing transaction for this item and add buyer
                    findAndUpdateTransaction(item, currentUserId);
                })
                .setNegativeButton("Cancel", null)
                .show();
    }

    // CHANGE 4: New method to find and update transaction with buyer
    private void findAndUpdateTransaction(Item item, String buyerId) {
        // Find transaction for this item where buyerUserID is null
        FirebaseRefs.pendingByUser(item.sellerUid).orderByChild("itemId").equalTo(item.id)
                .addListenerForSingleValueEvent(new ValueEventListener() {
                    @Override
                    public void onDataChange(@NonNull DataSnapshot snapshot) {
                        for (DataSnapshot txSnapshot : snapshot.getChildren()) {
                            Transaction t = txSnapshot.getValue(Transaction.class);
                            if (t != null && t.buyerUserID == null) {
                                // Update transaction with buyer
                                t.buyerUserID = buyerId;
                                t.buyerConfirmed = true; // Buyer confirms by bidding

                                FirebaseRefs.pendingByUser(item.sellerUid).child(t.id).setValue(t);
                                FirebaseRefs.pendingByUser(buyerId).child(t.id).setValue(t)
                                        .addOnSuccessListener(aVoid -> {
                                            toast(item.isFree ? "Bid placed! Waiting for seller to accept."
                                                    : "Bid placed! Waiting for seller to accept.");
                                        })
                                        .addOnFailureListener(e -> toast("Failed to place bid: " + e.getMessage()));
                                return;
                            }
                        }
                        toast("Could not find transaction for this item");
                    }

                    @Override
                    public void onCancelled(@NonNull DatabaseError error) {
                        toast("Error finding transaction: " + error.getMessage());
                    }
                });
    }
